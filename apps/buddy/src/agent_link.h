#pragma once
// Buddy-app glue between the transports and the agent-state lib: demo mode,
// liveness tracking, RTC time-sync, token→stats feed. Replaces the original
// data.h with the same public names so main.cpp call sites don't change.
// SINGLE-TU HEADER: include from main.cpp only — holds file-static state.
#include <Arduino.h>
...
// _applyLine can route {"cmd":...} docs before state parsing.
